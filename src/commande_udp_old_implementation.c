...
void receiveCarte(int socket) {
    ...
    for (int i = 0; i < carte.row; i++) {
        for (int j = 0; j < carte.col; j++) {
            recv(socket, &carte.grid[i][j], sizeof(int), 0);
        }
    }
    //afficher_map_drones(); (if needed)
}
...
padin.sin_port   = htons(LEPORT);
    ...
    sendFile(s, &padin, "commande_drone.txt");
    ...
    *p_lgexp1 = sizeof(p_exp);
    ...
    if (received_bytes > 0) {
        ...
        //bd = recvfrom(s, buffer, sizeof(buffer), 0, (struct sockaddr *)&p_exp, &taille);

        // Check for termination message
        if (strcmp(buffer, "END_OF_DRONE_COMMANDS") == 0) {
            printf("All drone commands executed.\n");
            goto end_loop; // ou bien break si une seul bouclee
        }
