...
void sendCarte(int socket, const struct sockaddr_in *p_exp) {
    // First, send the dimensions of the grid
    ...
    // Now send the grid row by row
    for (int i = 0; i < carte.row; i++) {
        sendto(socket, carte.grid[i], carte.col * sizeof(int), 0, (struct sockaddr *)p_exp, sizeof(struct sockaddr_in));
    }
}
